...
for a in [0]:
    for ss in range(len(file_name_list)):
        ...
        for t in range(N_POINTS):
            fname = s+"-"+str(t+5)+"-"+"50"
            data_file = os.path.join(current_dir, "sim_all_bler","sim_all_bler-2024-July-17-00-18-18-ail",fname)
            data = np.genfromtxt(data_file, delimiter=',')
            data = data[0:N_REPEAT, 5:]
...
for a in [1]:
    for ss in range(len(file_name_list)):
        ...
        for t in range(N_POINTS):
            ...
            data = data[0:N_REPEAT, 5:]
            data = np.sort(data,axis=1)[:, ::-1]
            data = np.mean(np.mean(data[:,0:20]))
            data_point[t] = data
        line, = axs[a].plot(np.arange(5,5+N_POINTS)*20, data_point, marker=markers[ss],linewidth=1,markerfacecolor='none')
        lines.append(line)
        axs[a].set_position([0.16+a*0.455, 0.215, 0.35, 0.6])
