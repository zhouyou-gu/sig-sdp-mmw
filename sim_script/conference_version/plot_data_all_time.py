...
file_name_list = ["time-6-50","time-8-50","time-10-50"]
...
data_point = np.zeros((N_POINTS,len(data_name_list)))
for t in range(N_POINTS):
    data_file = os.path.join(current_dir, "sim_all_time","sim_all_time-2024-July-17-17-27-43-ail",file_name_list[t])
    data = np.genfromtxt(data_file, delimiter=',')
    data = np.mean(data[:,2:],axis=0)/1e6
    data_point[t] = data

index = np.asarray([6,8,10])
bar_width = 0.4
bars = []
for i in range(len(data_name_list)):
    b = axs.bar(index + i * bar_width - (len(data_name_list)-1)*bar_width/2., data_point[:, i], bar_width, label=data_name_list[i],hatch=bar_fill_patterns[i])
    bars.append(b)
...
axs.set_ylim(0, 1)
# axs[1].set_ylim(0, 10)
axs.set_xticks([6,8,10])
axs.set_xticklabels([x*20 for x in [6,8,10]])
