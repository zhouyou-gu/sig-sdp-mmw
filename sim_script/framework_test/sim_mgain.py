...
for seed in range(REPEAT):
    ...
    z_vec, Z_fin, remainder  = MAX_GAIN.run(e.n_sta,state=(e.generate_S_Q_hmax()))
    bler = e.evaluate_bler(z_vec, Z_fin)
    mbler = np.mean(bler)
    wbler = np.max(bler)
    log.log_mul_scalar(data_name="mgain-"+str(CELL_SIZE)+"-"+str(int(RHO*10000)),iteration=seed,values=[Z_fin,mbler,wbler,bler])