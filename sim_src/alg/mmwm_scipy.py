...
class mmwm_scipy(feasibility_check_alg,StatusObject):
    ...
    def run_fc_main(self, Z, num_iterations=None) -> bool:
        ...
        for i in range(num_iterations):
            ...
            self._add_np_log("pct",np.asarray([p, p]),g_step=i)
    ...
    def run_fc_exphalffc_plug(self, Z, num_iterations=None, exphalffc=None) -> bool:
        ...
        for i in range(num_iterations):
            ...
            print("min max X_offdi", np.max(X_offdi.data), np.min(X_offdi.data),np.argmax(X_offdi.data), np.argmin(X_offdi.data))
            print("min max X_mdiag", np.max(X_mdiag.data), np.min(X_mdiag.data),np.argmax(X_mdiag.data), np.argmin(X_mdiag.data))
            X = X_offdi+X_mdiag
            ...
            print(i,"PHO_T_I_X",sa_T[0],sa_I[0],0)
            print(i,"ERR_T_I_X",T_violation_err,I_violation_err,0)
            print(i,"ECT_T_I_X",T_violation_cnt,I_violation_cnt,0)
            print(i,"V_I_TRUE_CNT",np.sum((I[I_violation_idx]+self.I[I_violation_idx])/self.I_max[I_violation_idx] > 1.))
    ...
    @profile
    def run_fc_exphalffc_plug_indp(self, Z, num_iterations=None, exphalffc=None) -> bool:
        assert exphalffc is not None
        if num_iterations is None:
            num_iterations = self._get_niteration()
